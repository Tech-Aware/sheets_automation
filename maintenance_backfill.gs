...
function registerStockActionsMenu_(ui) {
  ...
  stockMenu
    ...
    .addSeparator()
    .addItem('Harmoniser les SKU (Stock ← Achats)', 'harmonizeStockSkus')
...
function bulkValidateStockSelection() {
  ...
  if (!sh || sh.getName() !== 'Stock') {
    ss.toast('Ouvre la feuille "Stock" puis sélectionne la colonne "Valider".', 'Validation en lot', 6);
    return;
  }

  const layoutName = SHEET_LAYOUT.STOCK ? 'STOCK' : 'Stock';
  const headerRow = getSheetHeaderRow_(layoutName);
  const dataStartRow = getSheetDataStartRow_(layoutName);
  const lastRow = sh.getLastRow();
  if (lastRow < dataStartRow) {
    ss.toast('Aucune ligne à traiter dans "Stock".', 'Validation en lot', 6);
    return;
  }

  const headers = getSheetHeaders_(sh, layoutName);
  const resolver = makeHeaderResolver_(headers);
  ...
  const C_VALIDE = colExact(HEADERS.STOCK.VALIDER_SAISIE)
    ...
    || colWhere(h => h.includes('valid')); // fallback large (ex: "Validation saisie")
  ...
  const C_DMS = colExact(HEADERS.STOCK.DATE_MISE_EN_STOCK)
    || colExact(HEADERS.STOCK.DATE_MISE_EN_STOCK_ALT2)
    || colWhere(h => h.includes('mise en stock'));

  logDebug_('bulkValidateStockSelection.columns', {
    headerRow,
    dataStartRow,
    headersPreview: headers.slice(0, 20),
    resolved: {
      C_VALIDE, C_ID, C_LABEL, C_SKU, C_PRIX, C_TAILLE, C_LOT, C_STAMPV,
      combinedFlags: { useCombinedMisCol, useCombinedPubCol, useCombinedVenduCol },
      chronology: { C_DMS, C_DMIS, C_DPUB, C_DVENTE }
    },
    normalizedHeaders: resolver.normalized ? resolver.normalized.slice(0, 20) : []
  });

  if (!C_VALIDE) {
    logDebug_('bulkValidateStockSelection.error', {
      reason: 'C_VALIDE missing',
      headersPreview: headers.slice(0, 25),
      normalizedHeaders: resolver.normalized ? resolver.normalized.slice(0, 25) : []
    });
    ss.toast('Colonne "Valider la saisie" introuvable dans Stock.', 'Validation en lot', 8);
    return;
  }
  if (!C_VENDU || !C_DVENTE) {
    logDebug_('bulkValidateStockSelection.error', {
      reason: 'C_VENDU or C_DVENTE missing',
      C_VENDU,
      C_DVENTE,
      headersPreview: headers.slice(0, 25)
    });
    ss.toast('Colonnes "Vendu" ou "Date de vente" introuvables pour valider la sélection.', 'Validation en lot', 8);
    return;
  }
  if (!C_PRIX) {
    logDebug_('bulkValidateStockSelection.error', {
      reason: 'C_PRIX missing',
      headersPreview: headers.slice(0, 25)
    });
    ss.toast(`Colonne ${HEADERS.STOCK.PRIX_VENTE} introuvable.`, 'Validation en lot', 8);
    return;
  }
  if (!C_TAILLE) {
    logDebug_('bulkValidateStockSelection.error', {
      reason: 'C_TAILLE missing',
      headersPreview: headers.slice(0, 25)
    });
    ss.toast(`Colonne ${tailleHeaderLabel} introuvable.`, 'Validation en lot', 8);
    return;
  }

  const selection = ss.getSelection();
  const rangeList = selection ? selection.getActiveRangeList() : null;
  const ranges = rangeList ? rangeList.getRanges() : (selection && selection.getActiveRange() ? [selection.getActiveRange()] : []);
  if (!ranges.length) {
    ss.toast('Sélectionne d’abord les cases "Valider" à cocher.', 'Validation en lot', 6);
    return;
  }
  ...
  for (let i = 0; i < ranges.length; i++) {
    ...
    if (range.getNumColumns() !== 1 || range.getColumn() !== C_VALIDE) {
      ss.toast('La sélection doit être limitée à la colonne "Valider".', 'Validation en lot', 7);
      return;
    }
    if (range.getRow() < dataStartRow) {
      ss.toast('La sélection doit démarrer à partir des lignes de données.', 'Validation en lot', 7);
      return;
    }
  }
  ...
  if (!shippingLookup) {
    ss.toast('Impossible de calculer les frais de colissage : configure la feuille "Frais".', 'Validation en lot', 8);
    return;
  }
  const baseToDmsMap = buildBaseToStockDate_(ss);
  const chronoCols = { dms: C_DMS, dmis: C_DMIS, dpub: C_DPUB, dvente: C_DVENTE };
  const rowSet = new Set();
  ranges.forEach(r => {
    const rowStart = r.getRow();
    const numRows = r.getNumRows();
    for (let offset = 0; offset < numRows; offset++) {
      rowSet.add(rowStart + offset);
    }
  });
  ...
  let success = 0;
  const errors = [];
  logDebug_('bulkValidateStockSelection.start', { rows, C_VALIDE, chronoCols });

  rows.forEach(rowIndex => {
    try {
      const vendu = isStatusActiveValue_(sh.getRange(rowIndex, C_VENDU).getValue());
      if (!vendu) {
        errors.push({ row: rowIndex, reason: 'La case "Vendu" est vide ou décochée.' });
        return;
      }

      const chronoCheck = enforceChronologicalDates_(sh, rowIndex, chronoCols, {
        requireAllDates: true,
        warnOnly: true,
        toastLabel: 'Validation en lot'
      });
      if (!chronoCheck.ok) {
        errors.push({ row: rowIndex, reason: chronoCheck.message || 'Chronologie manquante ou invalide.' });
        return;
      }

      if (!ensureValidPriceOrWarn_(sh, rowIndex, C_PRIX)) {
        errors.push({ row: rowIndex, reason: `Prix de vente invalide en colonne ${HEADERS.STOCK.PRIX_VENTE}.` });
        return;
      }

      const tailleValue = String(sh.getRange(rowIndex, C_TAILLE).getDisplayValue() || '').trim();
      if (!tailleValue) {
        errors.push({ row: rowIndex, reason: `Valeur manquante dans ${tailleHeaderLabel}.` });
        return;
      }
      const lotValue = C_LOT ? String(sh.getRange(rowIndex, C_LOT).getDisplayValue() || '').trim() : '';
      const fraisColis = shippingLookup(tailleValue, lotValue);
      if (!Number.isFinite(fraisColis)) {
        const lotMessage = lotValue ? ` / lot ${lotValue}` : '';
        errors.push({ row: rowIndex, reason: `Frais de colissage introuvables pour ${tailleValue}${lotMessage}.` });
        return;
      }

      const dateVente = sh.getRange(rowIndex, C_DVENTE).getValue();
      if (!(dateVente instanceof Date) || isNaN(dateVente.getTime())) {
        errors.push({ row: rowIndex, reason: 'Date de vente absente ou invalide.' });
        return;
      }

      const validation = sh.getRange(rowIndex, C_VALIDE).getDataValidation();
      if (!validation
        || validation.getCriteriaType() !== SpreadsheetApp.DataValidationCriteria.CHECKBOX
        || (validation.getAllowInvalid && validation.getAllowInvalid())) {
        const checkboxRule = SpreadsheetApp.newDataValidation().requireCheckbox().setAllowInvalid(false).build();
        sh.getRange(rowIndex, C_VALIDE).setDataValidation(checkboxRule);
      }
      sh.getRange(rowIndex, C_VALIDE).setValue(true);
      ...
      success++;
      logDebug_('bulkValidateStockSelection.rowSuccess', {
        ligne: rowIndex,
        taille: tailleValue,
        lot: lotValue,
        fraisColis,
      ...
      logDebug_('bulkValidateStockSelection.exception', { ligne: rowIndex, message, stack: err && err.stack });
    }
  });

  const errorPreview = errors.slice(0, 3).map(e => `L${e.row}: ${e.reason}`).join(' / ');
  const suffix = errorPreview
    ? ` ${errors.length} ligne(s) en erreur : ${errorPreview}${errors.length > 3 ? ' …' : ''}`
    : '';
  ss.toast(`${success} ligne(s) validée(s).${suffix}`, 'Validation en lot', 8);
  logDebug_('bulkValidateStockSelection.done', { success, errors });
}
