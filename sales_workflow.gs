function handleVentesReturn(e) {
  ...
  if (!ss || !sh || sh.getName() !== 'Ventes' || !range) {
    logDebug_('handleVentesReturn.skip', { reason: 'sheet/range invalid', sheet: sh && sh.getName && sh.getName() });
    return;
  }
  ...
  const newValue = e && e.value !== undefined ? e.value : range.getValue();

  logDebug_('handleVentesReturn.start', {
    sheet: sh && sh.getName && sh.getName(),
    row,
    col,
    headerRow,
    dataStartRow,
  ...
  if (row < dataStartRow) {
    logDebug_('handleVentesReturn.skip', { reason: 'header/data zone', row, dataStartRow });
    return;
  }

  if (!lastColumn) {
    logDebug_('handleVentesReturn.skip', { reason: 'no columns' });
    return;
  }
  ...
  if (!colReturn || col !== colReturn) {
    logDebug_('handleVentesReturn.skip', { reason: 'not return column', colReturn, col });
    return;
  }

  logDebug_('handleVentesReturn', { ligne: row, colonne: col, valeur: newValue });

  if (!isReturnFlagActive_(newValue)) {
    logDebug_('handleVentesReturn.skip', { reason: 'flag not active', value: newValue });
    return;
  }
  ...
  logDebug_('handleVentesReturn.sale', {
    id: sale && sale.id,
    sku: sale && sale.sku,
    libelle: sale && sale.libelle,
    dateVente: sale && sale.dateVente,
    lot: sale && sale.lot,
    taille: sale && sale.taille
  });

  // Nouveau code :
  if (!sale.dateVente) {
    logDebug_('handleVentesReturn.stop', { reason: 'dateVente missing' });
    clearReturnFlag_(range);
    ss.toast('Impossible de traiter le retour : date de vente introuvable.', 'Ventes', 8);
    logDebug_('handleVentesReturn', { ligne: row, statut: 'arrêt', raison: 'date de vente manquante' });
    return;
  }

  // NOUVEAU : l’ID est obligatoire pour réintégrer correctement le lot
  const saleId = sale && sale.id !== undefined && sale.id !== null ? String(sale.id).trim() : '';
  if (!saleId) {
    clearReturnFlag_(range);
    ss.toast(
      'Impossible de traiter le retour : ID manquant dans la vente. ' +
      'Renseigne la colonne "ID" sur cette ligne de Ventes puis recoche "Retour".',
      'Ventes',
      10
    );
    logDebug_('handleVentesReturn', { ligne: row, statut: 'arrêt', raison: 'ID manquant' });
    return;
  }

  // Normalise aussi l’objet sale pour le reste du flux (optionnel mais propre)
  sale.id = saleId;
  // --- END REPLACE BLOCK ---
  ...
  try {
    ...
    logDebug_('handleVentesReturn.ledger', { result: ledgerResult });

    if (!ledgerResult || !ledgerResult.removed) {
      clearReturnFlag_(range);
      ss.toast(`Impossible de retirer ${labelForToast} du journal mensuel.`, 'Ventes', 8);
      logDebug_('handleVentesReturn', { ligne: row, statut: 'arrêt', raison: 'journal non retiré', ledgerResult });
      return;
    }

    const returnedAt = new Date();
    const stockResult = restoreSaleToStock_(ss, Object.assign({}, sale, { returnedAt }));
    logDebug_('handleVentesReturn.restoreStock', stockResult);
    if (!stockResult.success) {
      ...
      logDebug_('handleVentesReturn', { ligne: row, statut: 'arrêt', raison: 'restoreStock échec', stockResult });
      return;
    }

    sh.deleteRow(row);
    ss.toast(`Retour traité pour ${labelForToast}.`, 'Ventes', 5);
    logDebug_('handleVentesReturn', { ligne: row, statut: 'succès', cibleStock: stockResult.row });
  } catch (err) {
    logDebug_('handleVentesReturn.exception', { message: err && err.message, stack: err && err.stack });
    clearReturnFlag_(range);
    ...
    logDebug_('handleVentesReturn', { ligne: row, statut: 'exception', erreur: message });
  }
}
...
function restoreSaleToStock_(ss, sale) {
  ...
  // Fallback pour retrouver la vraie ligne d’en-tête dans Stock (si déplacée)
  function resolveStockHeaderWithFallback_() {
    const maxProbe = 10;
    const lastCol = stock.getLastColumn();
    if (!lastCol) return { headers: [], resolver: makeHeaderResolver_([]), headerRow: 1, dataStartRow: 2, fallback: false };

    const tryRow = (row) => {
      const headers = stock.getRange(row, 1, 1, lastCol).getValues()[0];
      return { headers, resolver: makeHeaderResolver_(headers), headerRow: row, dataStartRow: row + 1 };
    };

    const baseRow = getSheetHeaderRow_('Stock') || 1;
    const base = tryRow(baseRow);
    const hasIdAndSku = (res) =>
      !!(res.colExact(HEADERS.STOCK.ID) || res.colWhere(h => normText_(h) === 'id')) &&
      !!(res.colExact(HEADERS.STOCK.SKU) || res.colExact(HEADERS.STOCK.REFERENCE) || res.colWhere(h => h.includes('sku') || h.includes('reference')));
    if (hasIdAndSku(base.resolver)) return Object.assign(base, { fallback: false });

    const limit = Math.min(maxProbe, stock.getLastRow() || maxProbe);
    for (let r = 1; r <= limit; r++) {
      const probe = tryRow(r);
      if (hasIdAndSku(probe.resolver)) {
        logDebug_('restoreSaleToStock_.fallbackHeader', { headerRow: r });
        return Object.assign(probe, { fallback: true });
      }
    }
    logDebug_('restoreSaleToStock_.noHeader', { triedRows: limit, lastCol });
    return Object.assign(base, { fallback: false });
  }

  const headerInfoStock = resolveStockHeaderWithFallback_();
  const headers = headerInfoStock.headers;
  const resolver = headerInfoStock.resolver;
  const colExact = resolver.colExact.bind(resolver);
  const colWhere = resolver.colWhere.bind(resolver);

  // --- récupération infos Achats (livraison et date de mise en stock depuis Achats si dispo) ---
  function getAchatInfo_(ss, idValue, skuValue) {
    ...
    const resA = makeHeaderResolver_(headersA);
    const colIdA = resA.colExact(HEADERS.ACHATS.ID);
    const colRefA = resA.colExact(HEADERS.ACHATS.REFERENCE) || resA.colWhere(h => h.includes('reference'));
    const colLivA = resA.colExact(HEADERS.ACHATS.DATE_LIVRAISON)
      || resA.colExact(HEADERS.ACHATS.DATE_LIVRAISON_ALT)
      || resA.colWhere(h => h.includes('livraison'));
    const colDmsA = resA.colExact(HEADERS.ACHATS.DATE_MISE_EN_STOCK)
      || resA.colExact(HEADERS.ACHATS.DATE_MISE_EN_STOCK_ALT)
      || resA.colWhere(h => h.includes('mise en stock') || h.includes('mis en stock'));

    if (!colIdA && !colRefA) return {};

    const data = achats.getRange(2, 1, lastRow - 1, achats.getLastColumn()).getValues();
    const keyId = idValue !== undefined && idValue !== null ? normText_(idValue) : '';
    const keySku = skuValue ? normText_(skuValue) : '';
    for (let i = 0; i < data.length; i++) {
      const row = data[i];
      const idMatch = keyId && colIdA ? normText_(row[colIdA - 1]) === keyId : false;
      const refMatch = keySku && colRefA ? normText_(row[colRefA - 1]) === keySku : false;
      if (!idMatch && !refMatch) continue;
      const livraison = colLivA ? getDateOrNull_(row[colLivA - 1]) : null;
      const dms = colDmsA ? getDateOrNull_(row[colDmsA - 1]) : null;
      return { livraison, dms };
    }
    return {};
  }

  const C_ID = colExact(HEADERS.STOCK.ID)
    || colWhere(h => h === 'id' || h === 'identifiant' || h.includes('id'));
  ...
  const C_DMS = colExact(HEADERS.STOCK.DATE_MISE_EN_STOCK) || colWhere(h => h.includes('mise en stock') || h.includes('mis en stock'));
  ...
  logDebug_('restoreSaleToStock_', {
    id: sale && sale.id,
    sku: sale && sale.sku,
    cible: { C_ID, C_SKU, C_LABEL, C_PRIX, C_TAILLE, C_LOT, C_DMS, C_STAMPV, C_VALIDE, C_DMIS, C_DPUB, C_DVENTE }
  });
  ...
  const returnedAt = sale && sale.returnedAt instanceof Date && !isNaN(sale.returnedAt)
    ? sale.returnedAt
    : new Date();
  const achatInfo = getAchatInfo_(ss, sale && sale.id, sale && sale.sku);
  ...
  // Livraisons depuis Achats
  // Exigence : dans Stock, la colonne LIVRAISON doit refléter la date de mise en stock d’Achats
  const C_LIVRAISON = colExact(HEADERS.STOCK.DATE_LIVRAISON) || colExact(HEADERS.STOCK.DATE_LIVRAISON_ALT2) || colWhere(h => h.includes('livraison'));
  if (C_LIVRAISON) {
    const livraisonDate = (achatInfo && achatInfo.dms instanceof Date && !isNaN(achatInfo.dms))
      ? achatInfo.dms
      : (achatInfo && achatInfo.livraison instanceof Date && !isNaN(achatInfo.livraison) ? achatInfo.livraison : returnedAt);
    setValue(C_LIVRAISON, livraisonDate);
  }
  ...
  if (C_DMS) {
    // Dans Stock, MIS EN STOCK doit être la date du clic RETOUR
  ...
  const targetRow = Math.max(headerInfoStock.dataStartRow, stock.getLastRow() + 1);
  stock.getRange(targetRow, 1, 1, lastColumn).setValues([rowValues]);
  logDebug_('restoreSaleToStock.resultRow', {
    targetRow,
    C_ID,
    C_SKU,
    C_LABEL,
    ...
    C_LIVRAISON,
    C_DMS,
    rowPreview: rowValues.slice(0, Math.min(12, rowValues.length))
  });
...
function exportVente_(e, row, C_ID, C_LABEL, C_SKU, C_PRIX, C_DVENTE, C_STAMPV, baseToDmsMap, options) {
  ...
  if (ventes.getLastRow() === 0) {
    ventes.getRange(ventesHeaderRow, 1, 1, DEFAULT_VENTES_HEADERS.length).setValues([DEFAULT_VENTES_HEADERS]);
  }

  const headerWidth = Math.max(DEFAULT_VENTES_HEADERS.length, ventes.getLastColumn());
  const headerRange = ventes.getRange(ventesHeaderRow, 1, 1, headerWidth);
  ...
  // Date vente
  const dateV = sh.getRange(row, C_DVENTE).getValue();
  if (!(dateV instanceof Date) || isNaN(dateV)) return;

  // Valeurs stock
  const idValRaw = C_ID ? sh.getRange(row, C_ID).getValue() : "";
  const idVal = (idValRaw === null || idValRaw === undefined) ? "" : String(idValRaw).trim();
  ...
  if (!idVal) {
    ss.toast(`Export vente annulé : colonne "${HEADERS.STOCK.ID}" vide sur la ligne ${row}.`, 'Stock', 8);
    return;
  }

  // Achat info (prix achat + date réception)
  ...
  // Colonnes chrono stock (pour calcul délais + check)
  function resolveStockHeadersWithFallback_() {
    const maxProbe = 10;
    const lastCol = sh.getLastColumn();
    if (!lastCol) {
      return { headers: [], resolver: makeHeaderResolver_([]), headerRow: 1, dataStartRow: 2, fallback: false };
    }

    function build(row) {
      const headers = sh.getRange(row, 1, 1, lastCol).getValues()[0];
      return { headers, resolver: makeHeaderResolver_(headers), headerRow: row, dataStartRow: row + 1 };
    }

    function hasChrono(resolver) {
      return !!(
        resolver.colExact(HEADERS.STOCK.DATE_MISE_EN_STOCK)
        || resolver.colExact(HEADERS.STOCK.DATE_MISE_EN_STOCK_ALT2)
        || resolver.colWhere(h => h.includes('mise en stock') || h.includes('mis en stock'))
      );
    }

    const layoutHeader = getSheetHeaderRow_('Stock');
    const candidate = build(layoutHeader);
    if (hasChrono(candidate.resolver)) {
      return Object.assign(candidate, { fallback: false });
    }

    const lastRow = sh.getLastRow();
    const upperBound = Math.min(maxProbe, lastRow || maxProbe);
    for (let r = 1; r <= upperBound; r++) {
      const probe = build(r);
      if (hasChrono(probe.resolver)) {
        logDebug_('exportVente_.fallbackHeader', { headerRow: r });
        return Object.assign(probe, { fallback: true });
      }
    }

    logDebug_('exportVente_.noChronoHeaders', { triedRows: upperBound, lastCol });
    return Object.assign(candidate, { fallback: false });
  }

  const headerInfo = resolveStockHeadersWithFallback_();
  const headersS = headerInfo.headers;
  const resolverS = headerInfo.resolver;

  const C_DMS = resolverS.colExact(HEADERS.STOCK.DATE_MISE_EN_STOCK)
    || resolverS.colExact(HEADERS.STOCK.DATE_MISE_EN_STOCK_ALT2) // "MIS EN STOCK"
    || resolverS.colWhere(h => h.includes('mise en stock'));
  ...
  const dMiseLigne = getDateCell_(row, C_DMIS);
  const dPub       = getDateCell_(row, C_DPUB);

  let dMiseStock = getDateCell_(row, C_DMS);
  ...
  logDebug_('exportVente_', {
    ligneStock: row,
    id: idVal,
    sku,
    dateVente: dateV,
    delais: { delaiImm, delaiML, delaiPub, delaiVte },
    colonnes: { C_DMS, C_DMIS, C_DPUB, C_DVENTE }
  });
  ...
  if (!Number.isFinite(prixAchat)) {
    logDebug_('exportVente_.prixAchatManquant', {
      id: idVal,
      sku,
      headerRowStock: headerInfo.headerRow || getSheetHeaderRow_('Stock'),
      headerRowVentes: ventesHeaderRow,
      prixAchat
    });
  }

  // Insert vente
  const start = Math.max(ventesDataStartRow, ventes.getLastRow() + 1);
  const newRow = Array(widthVentes).fill("");

  if (COL_ID_VENTE) newRow[COL_ID_VENTE - 1] = idVal;                 // <-- ID exact obligatoire
  ...
  // Copie en compta (inclut lot/taille pour cohérence et retours)
  copySaleToMonthlySheet_(ss, {
    ...
    nbPieces: 1,
    sku,
    lot: shipping && shipping.lot ? shipping.lot : '',
    taille: shipping && shipping.size !== undefined ? shipping.size : ''
  });
  ...
  if (lastV >= ventesDataStartRow && COL_DATE_VENTE) {
    const ventesHeight = lastV - ventesDataStartRow + 1;
    ventes.getRange(ventesDataStartRow, 1, ventesHeight, ventes.getLastColumn())
      .sort([{ column: COL_DATE_VENTE, ascending: false }]);
    ventes.getRange(ventesDataStartRow, COL_DATE_VENTE, ventesHeight, 1).setNumberFormat('dd/MM/yyyy');
  }
...
function getAchatsRecordByIdOrSku_(ss, idVal, sku) {
  ...
  function resolveAchatsHeaderWithFallback_() {
    const maxProbe = 10;
    const lastCol = achats.getLastColumn();
    const tryRow = (row) => {
      const headers = achats.getRange(row, 1, 1, lastCol).getValues()[0];
      return { headers, resolver: makeHeaderResolver_(headers), headerRow: row };
    };
    const baseRow = getSheetHeaderRow_('Achats') || 1;
    const base = tryRow(baseRow);
    const hasCore = (res) => res.colExact(HEADERS.ACHATS.ID) && (res.colExact(HEADERS.ACHATS.REFERENCE) || res.colWhere(h => h.includes('reference')));
    if (hasCore(base.resolver)) return Object.assign(base, { fallback: false });
    const limit = Math.min(maxProbe, lastRow);
    for (let r = 1; r <= limit; r++) {
      const probe = tryRow(r);
      if (hasCore(probe.resolver)) {
        logDebug_('getAchatsRecordByIdOrSku_.fallbackHeader', { headerRow: r });
        return Object.assign(probe, { fallback: true });
      }
    }
    logDebug_('getAchatsRecordByIdOrSku_.noHeader', { triedRows: limit, lastCol });
    return Object.assign(base, { fallback: false });
  }

  const headerInfo = resolveAchatsHeaderWithFallback_();
  const headers = headerInfo.headers;
  const resolver = headerInfo.resolver;
  const dataStartRow = headerInfo.headerRow + 1;

  const colId = resolver.colExact(HEADERS.ACHATS.ID);
  const colRef = resolver.colExact(HEADERS.ACHATS.REFERENCE) || resolver.colWhere(h => h.includes('reference'));
  const colPrix = resolver.colExact(HEADERS.ACHATS.PRIX_UNITAIRE_TTC)
    || resolver.colExact(HEADERS.ACHATS.PRIX_UNITAIRE_TTC_ALT)
    || resolver.colExact(HEADERS.ACHATS.PRIX_UNITAIRE_BRUTE)
    || resolver.colExact(HEADERS.ACHATS.PRIX_UNITAIRE_BRUTE_ALT)
    || resolver.colWhere(h => h.includes('prix') && h.includes('achat'));
  const colDate = resolver.colExact(HEADERS.ACHATS.DATE_LIVRAISON)
    || resolver.colExact(HEADERS.ACHATS.DATE_LIVRAISON_ALT)
    || resolver.colWhere(h => h.includes('livraison'));
  const colArticle = resolver.colExact(HEADERS.ACHATS.ARTICLE)
    || resolver.colExact(HEADERS.ACHATS.ARTICLE_ALT);
  ...
  const dataStart = Math.max(dataStartRow, 2);
  const dataHeight = Math.max(lastRow - dataStart + 1, 0);
  if (dataHeight <= 0) return null;

  const data = achats.getRange(dataStart, 1, dataHeight, achats.getLastColumn()).getValues();
  for (let i = 0; i < data.length; i++) {
    const row = data[i];
    ...
    let prixAchat = null;
    if (typeof prixCell === 'number' && Number.isFinite(prixCell)) {
      prixAchat = prixCell;
    } else {
      const parsed = toNumber_(prixCell);
    ...
    logDebug_('getAchatsRecordByIdOrSku_.match', {
  ...
  logDebug_('getAchatsRecordByIdOrSku_.notFound', {
    id: keyId,
    sku: keyRef,
...
function applyMonthlySheetFormats_(sheet) {
  ...
  // Colonne TAXES (format monétaire)
  const taxColIndex = typeof MONTHLY_LEDGER_INDEX.TAXES !== 'undefined' ? MONTHLY_LEDGER_INDEX.TAXES : -1;
  if (taxColIndex >= 0) {
    sheet.getRange(1, taxColIndex + 1, maxRows, 1).setNumberFormat('#,##0.00');
  }
  // Colonne COUT DE REVIENT (anciennement PRIX D'ACHAT)
  const costColIndex = typeof MONTHLY_LEDGER_INDEX.COUT_REVIENT !== 'undefined'
    ? MONTHLY_LEDGER_INDEX.COUT_REVIENT
    : MONTHLY_LEDGER_INDEX.PRIX_ACHAT;
  if (costColIndex >= 0) {
    sheet.getRange(1, costColIndex + 1, maxRows, 1).setNumberFormat('#,##0.00');
  }
  // Colonne BENEFICE NET (anciennement MARGE BRUTE)
  const profitColIndex = typeof MONTHLY_LEDGER_INDEX.BENEFICE_NET !== 'undefined'
    ? MONTHLY_LEDGER_INDEX.BENEFICE_NET
    : MONTHLY_LEDGER_INDEX.MARGE_BRUTE;
  if (profitColIndex >= 0) {
    sheet.getRange(1, profitColIndex + 1, maxRows, 1).setNumberFormat('#,##0.00');
  }
  if (MONTHLY_LEDGER_INDEX.NB_PIECES >= 0) {
...
function updateLedgerResultRow_(sheet, headersLen) {
  ...
  // Colonne E: Chiffre d'affaires (PRIX DE VENTE)
  if (MONTHLY_LEDGER_INDEX.PRIX_VENTE >= 0) {
  ...
  // Colonne F: Taxes (nouvelle colonne TAXES)
  const taxColIndex = typeof MONTHLY_LEDGER_INDEX.TAXES !== 'undefined' ? MONTHLY_LEDGER_INDEX.TAXES : -1;
  if (taxColIndex >= 0) {
    const taxCell = sheet.getRange(resultRowNumber, taxColIndex + 1);
  ...
  // Colonne G: Coût de revient (anciennement PRIX D'ACHAT, maintenant COUT DE REVIENT)
  const costColIndex = typeof MONTHLY_LEDGER_INDEX.COUT_REVIENT !== 'undefined'
    ? MONTHLY_LEDGER_INDEX.COUT_REVIENT
    : MONTHLY_LEDGER_INDEX.PRIX_ACHAT;
  if (costColIndex >= 0) {
    const costCell = sheet.getRange(resultRowNumber, costColIndex + 1);
    costCell.setValue(`Coût de revient : ${formatLedgerCurrencyLabel_(totalCost)}`);
    costCell.setNote('Coût de revient = somme des prix d\'achat + total des frais saisis (colonnes L à N).');
  }

  // Colonne H: Bénéfice net (anciennement MARGE BRUTE, maintenant BENEFICE NET)
  const profitColIndex = typeof MONTHLY_LEDGER_INDEX.BENEFICE_NET !== 'undefined'
    ? MONTHLY_LEDGER_INDEX.BENEFICE_NET
    : MONTHLY_LEDGER_INDEX.MARGE_BRUTE;
  if (profitColIndex >= 0) {
    const profitCell = sheet.getRange(resultRowNumber, profitColIndex + 1);
  ...
  if (LEDGER_FEES_COLUMNS.MONTANT > 0) {
    ...
    feeCell.setNote('Total cumulé des montants de frais saisis dans le tableau des colonnes L à N.');
  }
}
