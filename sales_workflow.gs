...
function findLedgerRowByIdentifiers_(sheet, headersLen, sale, dedupeKey, allowUpdates) {
  ...
  for (let i = 0; i < data.length; i++) {
    const row = data[i];
    if (ledgerRowDataMatchesSale_(row, sale)) {
      return i + 2;
    }
  }
...
function ledgerRowMatchesSale_(sheet, headersLen, rowNumber, sale) {
  if (!sheet || !Number.isFinite(rowNumber)) return false;
  const rowValues = sheet.getRange(rowNumber, 1, 1, headersLen).getValues()[0];
  return ledgerRowDataMatchesSale_(rowValues, sale);
}

function ledgerRowDataMatchesSale_(row, sale) {
