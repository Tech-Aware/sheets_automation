function handleAchats(e) {
  ...
  if (!target) {
    logDebug_('handleAchats.error', { message: 'Feuille Stock introuvable' });
    return;
  }

  // Repère dynamiquement les colonnes de Stock (en-tête à la ligne 6)
  const headersStock = getSheetHeaders_(target, 'STOCK');
  ...
  const C_DMS_STOCK     = resolverStock.colExact(HEADERS.STOCK.DATE_MISE_EN_STOCK)
    || resolverStock.colExact(HEADERS.STOCK.DATE_MISE_EN_STOCK_ALT2)
    || resolverStock.colWhere(h => h.includes('mise en stock'))
    || resolverStock.colWhere(h => h.includes('mis en stock')); // optionnel

  const base = skuBase;
  const label = `${article} ${marque} ${genre}`.trim();

  const lastExistingStockRow = target.getLastRow();
  const dataStartStock = STOCK_DATA_START_ROW;
  const width = Math.max(target.getLastColumn(), COL_LABEL_STOCK || 0, COL_SKU_STOCK || 0, COL_DATE_STOCK || 0, COL_ID_STOCK || 0);
  const achatIdKey = (achatId === null || achatId === undefined || achatId === '') ? '' : String(achatId);

  let existingStockDms = null;
  let maxSuffixForBase = 0;
  let existingCountForBaseAndId = 0;
  let totalExistingRows = 0;

  try {
    if (lastExistingStockRow >= dataStartStock && COL_SKU_STOCK) {
      const existingRows = lastExistingStockRow - dataStartStock + 1;
      const existingSkuValues = target.getRange(dataStartStock, COL_SKU_STOCK, existingRows, 1).getValues();
      let existingDmsValues = null;
      if (C_DMS_STOCK) {
        existingDmsValues = target.getRange(dataStartStock, C_DMS_STOCK, existingRows, 1).getValues();
      }
      let existingIdValues = null;
      if (COL_ID_STOCK) {
        existingIdValues = target.getRange(dataStartStock, COL_ID_STOCK, existingRows, 1).getValues();
      }

      const prefix = `${base}-`;
      totalExistingRows = existingRows;

      for (let i = 0; i < existingSkuValues.length; i++) {
        const rawSku = normalizeSkuBase_(existingSkuValues[i][0]);
        const rowIdRaw = existingIdValues && existingIdValues[i] ? existingIdValues[i][0] : '';
        const rowId = (rowIdRaw === null || rowIdRaw === undefined || rowIdRaw === '') ? '' : String(rowIdRaw);
        const suffix = extractSkuSuffix_(rawSku, base);

        if (suffix) {
          maxSuffixForBase = Math.max(maxSuffixForBase, suffix);
        }

        const baseMatches = rawSku && rawSku.indexOf(prefix) === 0;
        const idMatches = achatIdKey ? (rowId === achatIdKey) : false;

        if (baseMatches && idMatches) {
          existingCountForBaseAndId++;
          if (existingDmsValues && !existingStockDms && C_DMS_STOCK) {
    ...
    logDebug_('handleAchats.analyseStock.error', {
  ...
  const rowsToCreate = Math.max(0, qty - existingCountForBaseAndId);
  if (rowsToCreate <= 0) {
    if (C_DMS_STOCK && existingStockDms) {
      stpCell.setValue(existingStockDms);
    }
    logDebug_('handleAchats.skipCreation', {
      base,
      achatId: achatIdKey,
      quantiteRecue: qty,
      dejaExistants: existingCountForBaseAndId,
      totalStock: totalExistingRows
    });
  ...
  try {
    const rows = Array.from({length: rowsToCreate}, () => Array(Math.max(1, width)).fill(""));
    for (let i = 0; i < rows.length; i++) {
      ...
      if (C_DMS_STOCK) rowValues[C_DMS_STOCK - 1] = miseStockDate;
    }
    ...
    if (C_DMS_STOCK) {
      target.getRange(start, C_DMS_STOCK, rows.length, 1).setNumberFormat("dd/MM/yyyy");
    } else {
      logDebug_('handleAchats.creation.warn', {
        message: 'Colonne DATE DE MISE EN STOCK introuvable, aucune date écrite',
        base,
        achatId: achatIdKey
      });
    }
    if (COL_DATE_STOCK) {
      target.getRange(start, COL_DATE_STOCK, rows.length, 1).setNumberFormat("dd/MM/yyyy");
    }

    logDebug_('handleAchats.creation', {
      base,
      achatId: achatIdKey,
      quantiteRecue: qty,
      lignesCreees: rowsToCreate,
      suffixDepart: maxSuffixForBase + 1,
      stockDataStart: STOCK_DATA_START_ROW,
      insertionStartRow: start
    });

    renumberStockByBrand_();
  } catch (err) {
    logDebug_('handleAchats.creation.error', {
      message: err && err.message ? err.message : err,
      base,
      achatId: achatIdKey
    });
    ss.toast('Erreur lors de la création des lignes dans Stock. Consulte les journaux.', 'Achats → Stock', 8);
  }
}
...
function renumberStockByBrand_(onlyOld) {
  ...
  const COL_OLD   = resolver.colExact(HEADERS.STOCK.OLD_SKU) || 0;
  ...
  const width = Math.max(COL_NEW || 0, COL_OLD || 0, COL_ID || 0, stock.getLastColumn());
  ...
  for (let i = 0; i < data.length; i++) {
    ...
    const oldSku = COL_OLD ? String(row[COL_OLD - 1] || "").trim() : "";  // SKU ancienne si présente
